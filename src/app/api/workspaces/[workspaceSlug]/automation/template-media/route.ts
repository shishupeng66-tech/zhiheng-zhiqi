...
/**
 * 模板分步生成 · 第二步：使用用户确认的文案匹配素材。
 * POST /api/workspaces/:slug/automation/template-media
 * body: { templateId, businessContext, explicitTextPlan }
 */
export async function POST(
  ...
  if (!result.ok) {
    return NextResponse.json({ ok: false, error: result.error || '素材匹配失败' }, { status: 500 });
  }
