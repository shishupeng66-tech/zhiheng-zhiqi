...
def get_volcengine_tts_voices() -> list[str]:
    voices_with_gender = [
        ("zh_female_xiaohe_uranus_bigtts", "Female"),
        ("zh_female_vv_uranus_bigtts", "Female"),
        ("zh_male_m191_uranus_bigtts", "Male"),
        ("zh_male_taocheng_uranus_bigtts", "Male"),
        ("zh_female_peiqi_uranus_bigtts", "Female"),
        ("zh_male_ruyayichen_uranus_bigtts", "Male"),
    ]
    return [f"volcengine:{voice}-{gender}" for voice, gender in voices_with_gender]
...
def is_volcengine_tts_voice(voice_name: str) -> bool:
...
def tts(
    ...
    if is_volcengine_tts_voice(voice_name):
        parts = voice_name.split(":", 1)
        if len(parts) >= 2 and parts[1].strip():
            speaker = parts[1].rsplit("-", 1)[0].strip()
            return volcengine_tts(
                text, speaker, voice_rate, voice_file, voice_volume
            )
        logger.error(f"Invalid VolcEngine TTS voice name format: {voice_name}")
        return None
...
def volcengine_tts(
    ...
    api_key = str(config.volcengine_tts.get("api_key", "") or "").strip()
    if not api_key:
        logger.error("VolcEngine TTS API key is not set")
        return None

    base_url = (
        str(config.volcengine_tts.get("base_url", "") or "")
        .strip()
        .rstrip("/")
        or "https://openspeech.bytedance.com"
    )
    resource_id = (
        str(config.volcengine_tts.get("resource_id", "") or "").strip()
        or "seed-tts-2.0"
    )
    appid = (
        str(config.volcengine_tts.get("appid", "") or "").strip()
        or "haoming-smart-cut"
    )
    url = f"{base_url}/api/v3/tts/unidirectional"
    request_id = str(utils.get_uuid())
    speed_ratio = max(0.5, min(2.0, float(voice_rate or 1.0)))
    ...
    for i in range(3):
        try:
            logger.info(
                f"start VolcEngine TTS, resource_id: {resource_id}, speaker: {speaker}, try: {i + 1}"
            )
            ensure_file_path_exists(voice_file)
            ...
            if response.status_code != 200:
                logger.error(
                    f"VolcEngine TTS failed with status {response.status_code}: {response.text[:300]}"
                )
            ...
            audio_bytes = b""
            raw_text = response.content.decode("utf-8", errors="replace").strip()
            if raw_text:
                ...
                while index < len(raw_text):
                    ...
                    if index >= len(raw_text):
                        break
                    data, index = decoder.raw_decode(raw_text, index)
                    response_code = data.get("code")
                    if response_code not in (None, 0, 20000000):
                        logger.error(
                            "VolcEngine TTS failed with response code "
                            f"{response_code}: {data.get('message', '')}"
                        )
                        return None
            ...
            if not audio_bytes:
                logger.error(
                    f"VolcEngine TTS returned empty audio payload: {response.text[:300]}"
                )
                return None
...
def chatterbox_tts(
    ...
    payload = {
        ...
        "voice": voice,
        "response_format": "mp3",
    # OpenAI speech API accepts speed 0.25-4.0; the app rate is a
